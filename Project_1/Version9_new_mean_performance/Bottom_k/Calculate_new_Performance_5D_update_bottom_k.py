...
def get_worst_authors(df, start_date, end_date, k_percent):
    """
    ...
    # Select bottom K% by total_perf
    n_select = max(1, int(np.ceil(len(author_performance) * k_percent / 100)))
    return author_performance.nsmallest(n_select, 'total_perf')[['author']]
...
def run_rolling_analysis(df_train, df_test, start_date, end_date, k_percent, lookback_period, cost_per_trade, corpus_fraction, max_allocation_fraction, day_offset):
    ...
    while current <= end_date:
        ...
        best_authors_df = get_worst_authors(df_train, train_start, train_end, k_percent)
