...
def calculate_monthly_performance(df, best_authors_df, target_month, log_file):
    """
    ...
    with open(log_file, 'a') as f:
        ...
        for date_key, day_data in month_df.groupby('date'):
            ...
            for _, trade in day_data.iterrows():
                ...
                # f.write(f"    Author: {trade['author']}, Trade Return: {trade_return:.6f}, Allocation: {allocation_per_trade:.6f}\n")
...
def run_rolling_analysis(df_train, df_test, start_date, end_date, k_percent, lookback_period, log_file):
    """
    ...
    while current <= end_date:
        ...
        if not best_authors_df.empty:
            ...
            month_summary = calculate_monthly_performance(df_test, best_authors_df, current, log_file)
...
def main(df_train, df_test, start_date, end_date, lookback_period, k_percentages, output_dir):
    """
    Main analysis:
    ...
    log_file = os.path.join(output_dir, 'debug_log_no_update.txt')
    ...
    for k in tqdm(k_percentages, desc="Sweeping through K%"):
        monthly_df = run_rolling_analysis(df_train, df_test, start_date, end_date, k, lookback_period, log_file)
...
if __name__ == "__main__":
    ...
    end_date = "2022-12-31"
    lookback_period_range = np.arange(1, 2, 1)
    ...
    for i in lookback_period_range:
        ...
        results_df = main(df_d5, df_d5, start_date, end_date, i, k_percentages, "Output_1")
