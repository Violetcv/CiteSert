...
def calculate_monthly_performance(df, best_authors_df, target_month, cost_per_trade, corpus_fraction, max_allocation_fraction, day_offset, log_file):
    """
    ...
    • Use BDay(5) or Timedelta(days=5) for the holding period.
    ...
    # Initialize a dictionary to track pending returns
    pending_returns = {}
    allocated_funds = {}
    ...
    with open(log_file, 'a') as f:
        ...
        for date_key, day_data in month_df.groupby('date'):
            ...
            f.write(f"  Date: {date_key.strftime('%Y-%m-%d')}\n")
            for _, trade in day_data.iterrows():
                ...
                f.write(f"    Author: {trade['author']}, Trade Return: {trade_return:.6f}, Allocation: {allocation_per_trade:.6f}, Total Allocation Out Today: {total_allocation_out_today}\n")
            ...
            # print(f"pending_returns[return_date] : {pending_returns[return_date]}")
            allocated_funds[return_date] += total_allocation_out_today
            # print(f"allocated_funds[return_date] : {allocated_funds[return_date]}")
        ...
        f.write(f"Corpus Return: {corpus_return}\n")

        combined_metric = (monthly_mean_performance - cost_per_trade) * total_trades
        f.write(f"Combined Metric: {combined_metric}\n")
...
def run_rolling_analysis(df_train, df_test, start_date, end_date, k_percent, lookback_period, cost_per_trade, corpus_fraction, max_allocation_fraction, day_offset, log_file):
    """
    ...
    while current <= end_date:
        ...
        if not best_authors_df.empty:
            ...
            month_summary = calculate_monthly_performance(df_test, best_authors_df, current, cost_per_trade, corpus_fraction, max_allocation_fraction, day_offset, log_file)
...
def main(df_train, df_test,start_date,end_date,lookback_period, k_percentages, cost_per_trade, corpus_fraction, max_allocation_fraction, day_offset, output_dir):
    ...
    for k in tqdm(k_percentages, desc="Sweeping through K%"):
        monthly_df = run_rolling_analysis(df_train, df_test, start_date, end_date, k, lookback_period, cost_per_trade, corpus_fraction, max_allocation_fraction, day_offset, log_file)
...
if __name__ == "__main__":
    ...
    lookback_period_range = np.arange(12, 13, 1)
