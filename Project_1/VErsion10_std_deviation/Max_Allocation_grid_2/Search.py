...
def get_top_n_global_max(folder_path, top_n=5):
    """
    Reads all CSV files in the given folder, 
    and for those that contain the column 'max_monthly_corpus_return',
    adds a 'filename' column, then combines them. 
    The resulting combined DataFrame is sorted in descending order by the 
    'max_monthly_corpus_return' column. The function returns the top n rows 
    ...
    for file in csv_files:
        ...
        if 'max_monthly_corpus_return' not in df.columns:
            print(f"File {file} does not contain 'max_monthly_corpus_return'. Skipping.")
            continue
    ...
    # Sort descending by max_monthly_corpus_return
    combined_df_sorted = combined_df.sort_values('max_monthly_corpus_return', ascending=False)
...
if __name__ == "__main__":
    ...
    top_rows = get_top_n_global_max(folder_path, top_n)
    if not top_rows.empty:
        output_file = os.path.join(folder_path, "global_top_max_results.txt")
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(f"Top {top_n} rows (global) sorted by max_monthly_corpus_return:\n")
