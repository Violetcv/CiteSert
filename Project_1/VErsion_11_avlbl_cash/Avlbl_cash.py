...
def main(df_train, df_test, start_date, end_date, lookback_period, k_percentages, 
    ...
    for k in tqdm(k_percentages, desc="Sweeping through K%"):
        ...
        # Plot monthly time series for each parameter combination if monthly_df is not empty.
        if not monthly_df.empty:
            plot_monthly_time_series(monthly_df, lookback_period, max_allocation_fraction, output_dir)
